...
class SafetyManager:
    ...
    def check_input(
        self, user_id: str, text: str, endpoint: str, policy: Optional[SafetyPolicy] = None
    ) -> Dict[str, Any]:
        """
        ...
        if policy.rate_limiting_enabled:
            ...
            if not rate_check["allowed"]:
                ...
                result["violations"].append(
                    {"type": "rate_limit", "severity": "medium", "details": rate_check}
                )
                self._log_violation(
                    "rate_limit", user_id, {"endpoint": endpoint, "details": rate_check}, "medium"
                )
                return result

        # Check input safety
        input_check = self.input_filter.check(text=text, redact_pii=policy.auto_redact_pii)

        # Add content violations
        for violation in input_check["content_violations"]:
            result["violations"].append(
                {"type": violation["type"], "severity": violation["severity"], "details": violation}
            )
        ...
        if input_check["has_pii"]:
            result["violations"].append(
                {"type": "pii_detected", "severity": "low", "details": input_check["pii_detected"]}
            )
    ...
    def check_output(
        ...
        if policy.output_safety_checks_enabled:
            output_check = self.output_filter.check(
                response=text, query=query, context=context, redact_pii=policy.auto_redact_pii
            )
            ...
            for violation in output_check["violations"]:
                ...
                self._log_violation(
                    violation["type"], user_id, {"violation": violation}, violation["severity"]
                )
            ...
            if output_check["pii_detected"]:
                ...
                self._log_violation(
                    "pii_in_output", user_id, {"pii": output_check["pii_detected"]}, "low"
                )
    ...
    def _log_violation(
        self, violation_type: str, user_id: str, details: Dict[str, Any], severity: str
    ):
        """
        ...
        # Log
        log_message = (
            f"Safety violation: {violation_type} (severity: {severity}) for user {user_id}"
        )
        if severity == "high":
    ...
    def get_violations(
        self, user_id: Optional[str] = None, severity: Optional[str] = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_user_violation_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_violations": sorted(
                user_violations, key=lambda x: x["timestamp"], reverse=True
            )[:5],
        }

    def check_user_thresholds(
        self, user_id: str, policy: Optional[SafetyPolicy] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {"user_id": user_id, "exceeded_thresholds": exceeded, "blocked": len(exceeded) > 0}
    ...
    def cleanup_old_violations(self, max_age_hours: int = 24):
        ...
        self.violations = [
            v for v in self.violations if datetime.fromisoformat(v["timestamp"]) > cutoff
        ]
        ...
        for user_id in self.user_violations:
            self.user_violations[user_id] = [
                v
                for v in self.user_violations[user_id]
