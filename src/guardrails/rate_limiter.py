...
class TokenBucket:
    ...
    def __init__(self, capacity: int, refill_rate: int, refill_interval: int = 60):
        """
    ...
    def refill(self):
        ...
        if refills > 0:
            ...
            logger.debug(
                f"Refilled {tokens_to_add} tokens (bucket now has {self.tokens}/{self.capacity})"
            )
    ...
    def consume(self, tokens: int = 1) -> Dict[str, Any]:
        ...
        if self.tokens >= tokens:
            ...
            self.tokens -= tokens
            return {"allowed": True, "remaining_tokens": self.tokens, "retry_after": 0}
        else:
            # Not enough tokens
            ...
            return {"allowed": False, "remaining_tokens": self.tokens, "retry_after": retry_after}


class RateLimiter:
    """Rate limiter using token bucket algorithm"""

    def __init__(self, tokens_per_minute: int = 60, burst_size: Optional[int] = None):
        """
        ...
        logger.info(
            f"Rate limiter initialized: {tokens_per_minute} tokens/min, burst {self.burst_size}"
        )
    ...
    def _get_user_bucket(self, user_id: str) -> TokenBucket:
        ...
        if user_id not in self.user_buckets:
            self.user_buckets[user_id] = TokenBucket(
                capacity=self.burst_size, refill_rate=self.tokens_per_minute
            )
    ...
    def check_limit(self, user_id: str, endpoint: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if endpoint:
            ...
            if bucket_key not in self.user_buckets:
                self.user_buckets[bucket_key] = TokenBucket(
                    capacity=min(self.burst_size, endpoint_limit * 2), refill_rate=endpoint_limit
                )
    ...
    def consume_token(self, user_id: str, tokens: int = 1, endpoint: Optional[str] = None) -> bool:
        """
