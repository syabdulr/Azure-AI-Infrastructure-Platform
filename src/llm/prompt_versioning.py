...
class PromptVersionManager:
    ...
    def add_version(
        ...
        prompt_version = PromptVersion(
            version=version, template=template, metadata=metadata, is_active=set_as_active
        )
    ...
    def list_versions(self, template_name: str) -> List[str]:
        ...
        return sorted(self.templates[template_name].keys(), key=lambda v: self._parse_version(v))
    ...
    def record_metrics(self, template_name: str, version: str, metrics: Dict[str, float]):
        """
    ...
    def compare_versions(
        self, template_name: str, versions: Optional[List[str]] = None
...
def initialize_default_templates():
    ...
    version_manager.add_version(
        "rag_user_cot", "v1", get_rag_user_cot_prompt(), set_as_active=False
    )
    version_manager.add_version(
        "rag_user_fewshot", "v1", get_rag_user_fewshot_prompt(), set_as_active=False
    )

    # Register chat templates
    version_manager.add_version("chat_system", "v1", get_chat_system_prompt(), set_as_active=True)
    version_manager.add_version(
        "chat_code_assistant", "v1", get_chat_code_assistant_prompt(), set_as_active=True
    )
    version_manager.add_version("chat_analyst", "v1", get_chat_analyst_prompt(), set_as_active=True)

    # Register specialized templates
    version_manager.add_version(
        "summarization", "v1", get_summarization_prompt(), set_as_active=True
    )
