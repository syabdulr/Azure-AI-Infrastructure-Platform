...
class SupportAutomation:
    ...
    async def classify_ticket(self, subject: str, description: str) -> Dict[str, Any]:
        ...
        raise NotImplementedError(
            "Production classification requires OpenAI. " "Enable demo_mode=True for demonstration."
        )
    ...
    def _demo_classify(self, subject: str, description: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Classified ticket: {category.value} / {priority.value} (confidence: {confidence})"
        )

        return {"category": category.value, "priority": priority.value, "confidence": confidence}
    ...
    async def create_ticket(
        ...
        logger.info(
            f"Created ticket {ticket.ticket_id}: {ticket.category.value} / {ticket.priority.value}"
        )
    ...
    async def generate_response_suggestion(self, ticket: SupportTicket) -> ResponseSuggestion:
        """
    ...
    def _demo_generate_response(self, ticket: SupportTicket) -> ResponseSuggestion:
        ...
        template = response_templates.get(
            ticket.category, response_templates[TicketCategory.GENERAL]
        )
    ...
    async def get_ticket_analytics(
        self, tickets: Optional[List[SupportTicket]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def update_ticket_status(
        self, ticket_id: str, status: TicketStatus, assigned_to: Optional[str] = None
