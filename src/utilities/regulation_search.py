...
class RegulationSearch:
        ...
        for doc in self.documents:
            ...
            if relevance > 0.2:  # Only include reasonably relevant results
                ...
                result = SearchResult(
                    ...
                    snippet=snippet,
                    requirements=[req for req in doc.requirements if 
                                 any(word in req.lower() for word in query_words)],
    ...
    async def get_compliance_checklist(
        ...
        relevant_docs = [
            ...
            if (
                (doc.metadata.get("utility_type") == utility_type or 
                 doc.metadata.get("utility_type") == "all") and
                doc.metadata.get("jurisdiction").lower() == jurisdiction.lower()
            )
        ]
    ...
    async def get_regulation_timeline(
        ...
        sorted_docs = sorted(
            filtered_docs,
            key=lambda d: d.effective_date or datetime.min
        )
