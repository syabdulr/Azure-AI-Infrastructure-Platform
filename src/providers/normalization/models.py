...
class LogProb(BaseModel):
    ...
    top_logprobs: List[Dict[str, float]] = Field(
        ...
        description="Top alternative tokens and their logprobs"
    )
...
class NormalizedResponse(BaseModel):
    ...
    finish_reason: Optional[str] = Field(
        None,
        description="Why generation stopped (stop, length, content_filter, etc.)"
    )
    quality_score: Optional[float] = Field(
        None,
        description="LLM-as-a-judge quality score (0-1)",
        ge=0.0,
        le=1.0
    )
    ...
    tool_calls: List[ToolCall] = Field(
        default_factory=list,
        description="Tool/function calls in the response"
    )
    ...
    normalization_warnings: List[str] = Field(
        default_factory=list,
        description="Warnings from normalization process"
    )
