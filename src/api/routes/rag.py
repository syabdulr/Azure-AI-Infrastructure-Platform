...
from src.api.schemas import ErrorCode, ErrorResponse, RAGRequest, RAGResponse, SourceDocument
...
async def rag_query(request: RAGRequest) -> RAGResponse:
    ...
    try:
        ...
        # Record metrics
        record_request_metrics(tokens=tokens_used, cost=cost, latency_ms=latency_ms, error=False)
...
async def index_document(document: dict) -> dict:
    ...
    try:
        ...
        if not document.get("content"):
            raise HTTPException(status_code=400, detail="Document must have 'content' field")
        ...
        raise HTTPException(
            status_code=500, detail={"error": "Document indexing failed", "message": str(e)}
        )


@router.post("/index/batch")
async def batch_index_documents(documents: list, batch_size: int = 100) -> dict:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail={"error": "Batch indexing failed", "message": str(e)}
        )


def _build_context(search_results: List[Dict[str, Any]], context_window: int = 4000) -> str:
    """
