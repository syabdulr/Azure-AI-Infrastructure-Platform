...
async def check_input_safety(
    request: dict, x_user_id: Optional[str] = Header(None, alias="X-User-ID")
) -> Dict[str, Any]:
    """
    Check input safety
    ...
    try:
        ...
        # Check input safety
        result = safety_manager.check_input(user_id=user_id, text=text, endpoint=endpoint)

        latency_ms = (datetime.utcnow() - start_time).total_seconds() * 1000

        record_request_metrics(tokens=0, cost=0.0, latency_ms=latency_ms, error=False)

        return {**result, "checked_at": datetime.utcnow().isoformat(), "latency_ms": latency_ms}
...
async def check_output_safety(
    ...
    try:
        ...
        result = safety_manager.check_output(
            user_id=user_id, text=text, query=query, context=context
        )
...
async def get_rate_limit_status(user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {**status, "checked_at": datetime.utcnow().isoformat(), "latency_ms": latency_ms}
...
async def get_violations(
    ...
    try:
        ...
        # Get violations
        violations = safety_manager.get_violations(user_id=user_id, severity=severity, limit=limit)

        # Get total count (without limit)
        all_violations = safety_manager.get_violations(
            user_id=user_id, severity=severity, limit=10000
        )
        ...
        return {
            ...
            "filters": {"user_id": user_id, "severity": severity, "limit": limit},
