#!/usr/bin/env python3
"""
Send enterprise scenario visual + draft post to LinkedInProfMediaAgent bot.
"""
...
print("Sending enterprise visual...")
with open('/tmp/phase2_enterprise_visual.png', 'rb') as photo:
    photo_resp = requests.post(
        f"{BASE}/sendPhoto",
        data={
            "chat_id": chat_id,
            "caption": "📊 ENTERPRISE DEMO: Northbank Financial's AI Gateway routing requests across Trading, Support, and Compliance departments — with cache hits, budget alerts, A/B test results, and live Prometheus metrics."
        },
        files={"photo": photo}
    )
...
if photo_resp.status_code == 200 and text_resp.status_code == 200:
    print("\n✅ Enterprise visual + draft post sent to LinkedInProfMediaAgent!")
else:
